...
import cn.v5cn.netty.im.rest.spi.domain.LdapUser;
...
public class LdapUserSpiImpl implements UserSpi<LdapUser> {
    ...
    public LdapUser getUser(String username, String pwd) {
        ...
        final boolean authenticate = ldapTemplate.authenticate(searchBase, filter.encode(), pwd);
    ...
    public LdapUser getById(String id) {
        return ldapTemplate.searchForObject(ldapQueryCriteria().and(loginIdAttrName).is(id), ldapUserInfoMapper);
    }
