...
import cn.v5cn.springboot.websocket.handler.MessageHandler;
...
public class WebSocketHandler extends TextWebSocketHandler implements InitializingBean {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        // 解析 accessToken
        String accessToken = String.valueOf(session.getAttributes().get("accessToken"));
        // 创建 AuthRequest 消息类型
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        logger.info("[handleMessage][session({}) 接收到一条消息({})]", session, message); // 生产环境下，请设置成 debug 级别
        try {
            // 获得消息类型
            ...
            if(messageHandler == null) {
                logger.error("[onMessage][消息类型({}) 不存在消息处理器]", msgType);
                return;
            }
            // 解析消息
            Class<? extends Message> messageClass = this.getMessageClass(messageHandler);
            // 处理消息
            Message msgObj = JSON.parseObject(jsonMsg.getString("body"), messageClass);
            messageHandler.execute(session, msgObj);
