...
public class IpAddressUtil {
     ...
     * 查了查询地址API
     */
    private static final String ALI_IP_SEARCH_URL = "http://api01.aliyun.venuscn.com/ip?ip=%s";
    private static final String ALI_IP_SEARCH_URL_CODE = "461535aabeae4f34861884d392f5d452";
    ...
    public static String getAddress(HttpServletRequest request) {
        String resultJson = Constants.DASH;
        ...
        try {
            //获取阿里云定位api接口
            String api = ALI_IP_SEARCH_URL;//ConstantContextHolder.getIpGeoApi();
            //获取阿里云定位appCode
            String appCode = ALI_IP_SEARCH_URL_CODE;//ConstantContextHolder.getIpGeoAppCode();
            if (ObjectUtil.isAllNotEmpty(api, appCode)) {
                ...
                http.header(Constants.AUTHORIZATION, appCodeSymbol + " " + appCode);
                resultJson = http.timeout(3000).execute().body();
                resultJson = String.join("", (List<String>) JSONPath.eval(resultJson, path));
            }
        } catch (Exception e) {
            resultJson = Constants.DASH;
